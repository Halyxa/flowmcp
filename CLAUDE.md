...
Production TypeScript MCP server. 25 tools. 430 tests. All green.
...
ON_CI_FAILURE(stage, error):
  ...
  3. IF test fails → read failing test name + error message.
     ...
     f. Verify test count >= 430 (floor never drops).
  ...
  4. IF smoke test fails → read which of 15 checks failed.
     ...
     c. IF fix touches shared code (csv-utils.ts, fetchWithTimeout):
        ...
        BECAUSE shared code has blast radius across all 25 tools.
  ...
  8. IF fix touched tool interface or shared code:
     Run MCP Inspector → verify 25 tools respond to list_tools.
...
ON_DEMO_PREP(audience):
  ...
  3. MCP Inspector: verify 25 tools respond to list_tools.
...
ON_RESPOND():
  ...
  3. IF test results → "430/430 passed" or "429/430 — 1 failure in X".
...
ON_ACTION(predicted_outcome):
  ...
  1. PREDICT: State expected outcome before acting. Include confidence (0.0-1.0).
     "430 tests pass (confidence: 0.95)." "Force layout converges <5s at 500 nodes (confidence: 0.7)."
...
| Path | Contents |
|------|----------|
| `src/index.ts` | Main server, 25 tools |
...
| `src/integration.test.ts` | 26 MCP protocol tests |
...
```bash
npm test             # 430 tests (unit + integration + benchmark + perf + search + v2 + v3)
