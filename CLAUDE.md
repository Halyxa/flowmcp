...
ON_NEW_TOOL(name, spec):
  ...
  5. Add to tools table in TOOLS-REFERENCE.md.
...
Append-only log. One JSON per line. Never edit existing lines.
Fields: id, timestamp, type, action, result, confidence, artifacts, ooda_phase.
Log: session start/end, test runs, builds, errors, predictions, API changes, skills created, low-confidence decisions.
Skip: routine reads, individual tool calls, intermediate OODA steps.
Feeds: SPINE-05 (api_change patterns), SPINE-08 (error history), SPINE-09 (Brier scores), SPINE-10 (crystallization triggers).

## Tools & Paths

70 tools, 3 prompts, 5 resources. Full inventory: `TOOLS-REFERENCE.md`.
...
Full file paths, sibling table, Inspector command: `TOOLS-REFERENCE.md`.
...
- CSV `.split(",")` breaks on quoted fields — always use state-machine parser (csv-utils.ts)
- PageRank measures INCOMING links — star graph direction matters
- Worker thread exports must be at module top level, never inside conditionals
...
- The 150-Instruction Ceiling: Claude reliably follows ~150 custom instructions. Beyond that, rules get buried. Keep CLAUDE.md lean. Move reference to TOOLS-REFERENCE.md.
- The 128k Budget: context window is 128k effective, not 200k. 36% reserved for safety margins, compaction overhead, and reasoning space. Plan sessions accordingly.
- The Cache Invariant: one character change to system prompt invalidates the entire prompt cache. Never inject dynamic content into system-level instructions. Use user messages for dynamic context.
