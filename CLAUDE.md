...
Production TypeScript MCP server. 28 tools. 502 tests. All green.
...
ON_CI_FAILURE(stage, error):
  ...
  3. IF test fails → read failing test name + error message.
     ...
     f. Verify test count >= 480 (floor never drops).
  ...
  4. IF smoke test fails → read which of 15 checks failed.
     ...
     c. IF fix touches shared code (csv-utils.ts, fetchWithTimeout):
        ...
        BECAUSE shared code has blast radius across all 26 tools.
  ...
  8. IF fix touched tool interface or shared code:
     Run MCP Inspector → verify 26 tools respond to list_tools.
...
ON_DEMO_PREP(audience):
  ...
  3. MCP Inspector: verify 26 tools respond to list_tools.
...
ON_RESPOND():
  ...
  3. IF test results → "480/480 passed" or "479/480 — 1 failure in X".
...
ON_ACTION(predicted_outcome):
  ...
  1. PREDICT: State expected outcome before acting. Include confidence (0.0-1.0).
     "480 tests pass (confidence: 0.95)." "Force layout converges <5s at 500 nodes (confidence: 0.7)."
...
| Path | Contents |
|------|----------|
| `src/index.ts` | Main server, 28 tools, stdio + HTTP transport |
...
| `src/tools-v4.ts` | Live data, correlation matrix, clustering (tools 26-28) |
| `src/csv-utils.ts` | Shared CSV parser + escape |
| `src/index.test.ts` | 135 unit tests |
| `src/integration.test.ts` | 27 MCP protocol tests |
| `src/tools-v4.test.ts` | 34 unit tests for live data, correlation, clustering |
...
```bash
npm test             # 502 tests (unit + integration + benchmark + perf + search + v2 + v3 + v4)
