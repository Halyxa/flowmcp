...
The world's first AI-to-3D data visualization bridge. An MCP server with **70 tools** that lets any AI assistant transform raw data into interactive 3D spatial visualizations via [Flow Immersive](https://flowimmersive.com).

**1200 tests. 29 smoke checks. Zero competitors in 3D viz MCP.**
...
Health check: `GET /health` returns `{"status":"ok","tools":70,"transport":"streamable-http"}`
...
```bash
npm test           # 1200 tests
npm run smoke-test # 29 standalone MCP checks
