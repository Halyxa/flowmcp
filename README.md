...
The world's first AI-to-3D data visualization bridge. An MCP server with **59 tools** that lets any AI assistant transform raw data into interactive 3D spatial visualizations via [Flow Immersive](https://flowimmersive.com).

**841 tests. Zero competitors in 3D viz MCP.**
...
Health check: `GET /health` returns `{"status":"ok","tools":59,"transport":"streamable-http"}`
...
| Tool | What it does |
|------|-------------|
| `flow_normalize_data` | Min-max or z-score normalization |
| `flow_standardize` | Statistical standardization |
| `flow_fill_missing` | Impute: constant, mean, median, mode, forward fill |
| `flow_interpolate_missing` | Linear/nearest/zero interpolation |
