...
| `flow_browse_flows` | Browse 4k+ public flows (no auth) |
| `flow_get_flow` | Full flow definition by selector (no auth) |
| `flow_list_templates` | 37 viz templates with column requirements |
| `flow_list_categories` | 8 categories (derived from templates) |
...
| Tool | What it does |
|------|-------------|
| `flow_quest_generator` | Procedural quests from data topology — turns analysis into guided missions |
| `flow_near_miss_detector` | Find patterns that almost hold — the interesting failures in your data |
| `flow_progressive_disclosure` | Fog-of-war layering — reveal data progressively by depth or theme |
| `flow_anomaly_explain` | Detective-story anomaly narratives — why outliers exist, not just where |
| `flow_insight_scorer` | Statistical peer review — score and rank insights by significance |
| `flow_waypoint_map` | GPS waypoints for data worlds — navigation anchors across dimensions |
| `flow_visor_mode` | Switch analytical lenses (5 visors: statistical, network, temporal, spatial, categorical) |
...
```bash
node scripts/demo-30-second.mjs      # 6-tool pipeline demo (describe → cluster → narrate → quest → stats)
node scripts/stress-test-force.mjs   # Force layout at scale: 1k → 50k nodes
node scripts/stress-test-tools.mjs   # Tool performance benchmark at increasing dataset sizes
```
...
| `celebrity-bryan-johnson.csv` | 16 nodes | Longevity network |
| `celebrity-joe-rogan.csv` | 25 nodes | Podcast guest network |
| `celebrity-taylor-swift.csv` | 30 nodes | Relationship + collaboration |

### Celebrity Network Demos

Pre-built in `demos/` — exercise the Holodeck Intelligence tools:

| Dataset | Nodes | Type |
|---------|-------|------|
| `taylor-swift-network.csv` | 28 | Music collaboration network |
| `elon-musk-network.csv` | 30 | Multi-domain company constellation |
| `einstein-network.csv` | 25 | Scientific collaboration network |
